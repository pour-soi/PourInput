...
class HidGestureListener:
    ...
    def update_extra_diverts(self, extra_diverts=None):
        ...
        with self._extra_diverts_lock:
            old_extra = self._extra_diverts
            old_keys = set(old_extra)
            ...
            for cid in removed:
                info = old_extra.get(cid, {})
                if info.get("held"):
                    cb = info.get("on_up")
                    if cb:
                        print(f"[HidGesture] Extra {_format_cid(cid)} UP (sync removal)")
                        try:
                            cb()
                        except Exception as exc:
                            print(f"[HidGesture] extra sync up callback error: {exc}")
            self._stale_extra_divert_cids.update(removed)
            self._extra_diverts = next_extra

        print(
            "[HidGesture] Extra diverts updated: "
            + (
                ", ".join(_format_cid(cid) for cid in sorted(next_keys))
    ...
    def _undivert(self):
        ...
        # Undivert extra CIDs
        with self._extra_diverts_lock:
            extra_cids = set(self._extra_diverts) | set(self._stale_extra_divert_cids)
            self._stale_extra_divert_cids.clear()
