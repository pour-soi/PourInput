    ...
    def from_dict(cls, data) -> "WindowsUpdatePlan":
        ...
        try:
            return cls(
                ...
                executable_name=str(data.get("executable_name") or APP_EXECUTABLE_NAME),
            )
    ...
    *,
    ...
    with zipfile.ZipFile(zip_path) as zf:
        ...
        if requirements.require_windows_app:
            if f"{root}/{APP_EXECUTABLE_NAME}" not in files:
                raise UpdateInstallError("missing_entrypoint", f"Update archive does not contain {APP_EXECUTABLE_NAME}.")
...
def locate_runtime(
    *,
    ...
    if system.startswith("win"):
        root = exe.parent
        if not (root / APP_EXECUTABLE_NAME).exists() or not (root / "_internal").exists():
    ...
    *,
    ...
    if not runtime.platform_key.startswith("windows"):
        ...
        return InstallPlan(
            ...
            False,
            "manual_fallback",
            f"A new {APP_NAME} release is available. Install manually on {system}.",
            asset,
            staged,
        )
    if not runtime.update_supported:
        return InstallPlan(
            ...
            False,
            "manual_fallback",
            f"{APP_NAME} cannot safely update this install automatically. Please install manually from the release page.",
            asset,
            staged,
        )
    return InstallPlan(
        ...
        True,
        "ready_to_install",
        f"{APP_NAME} is ready to install the verified update.",
        asset,
        staged,
    )
...
def validate_windows_update_plan(
    ...
    *,
    ...
    if plan.current_pid <= 0:
        reject()
    if plan.executable_name != APP_EXECUTABLE_NAME:
        reject()
    if not install_root.is_dir():
        reject()
    if not (install_root / APP_EXECUTABLE_NAME).is_file():
        reject()
    ...
    if staged_root.name != install_root.name:
        reject()
    if not (staged_root / APP_EXECUTABLE_NAME).is_file():
        reject()
