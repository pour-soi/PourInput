...
class Backend(QObject):
    ...
    def buttons(self):
        ...
        for key, name in button_names.items():
            ...
            long_key = long_press_mapping_key(key)
            long_aid = mappings.get(long_key, "none")
            idx += 1
            result.append({
                "key": key,
                "name": name,
                ...
                "supportsMultiAction": supports_multi_action(key),
                "longActionId": long_aid,
                "longActionLabel": _action_label(long_aid),
                "index": idx,
            })
        return result
